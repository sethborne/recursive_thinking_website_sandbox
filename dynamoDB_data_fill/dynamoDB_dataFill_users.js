    ...
    for(let i = 0; i < userArray.length; i += 1){
        ...
        function generateUserSkillArray(skillArray){
          ...
          for(let s = 0; s < skillArray.length; s += 1){
            // each item
            if(skillArray[s]['_usersWithSkill'].includes(currentIdsForUsers[i])){
              //true 
              let tempString = {
                "S": skillArray[s]['Id']
              };
        ...
        // skillsLanguages
        tempObj['PutRequest']['Item'][userArray[i][26][0]] = { "L": generateUserSkillArray(allProfileSkillsLanguage)};
